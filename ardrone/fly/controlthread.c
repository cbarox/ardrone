...
void *ctl_thread_main(void *data) {
  ...
  while (1) {
    ...
    if (setpoint.h == 0.00) {
      ...
      adj_pitch = pid_CalcD(&pid_pitch, setpoint.pitch - att.pitch, att.dt,
                            att.gy); //err positive = need to pitch down
      adj_yaw = pid_CalcD(&pid_yaw, setpoint.yaw - att.yaw, att.dt,
                          att.gz); //err positive = need to increase yaw to the left
      adj_h = pid_CalcD(&pid_h, setpoint.h - att.h, att.dt,
                        att.hv); //err positive = need to increase height
