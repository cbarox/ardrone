...
int video_Init(struct vid_struct *vid) {
  ...
  vid->buffers = (struct buffer_struct *) calloc(vid->n_buffers,
  ...
  for (i = 0; i < vid->n_buffers; ++i) {
    ...
    vid->buffers[i].buf = mmap(NULL, buf.length, PROT_READ | PROT_WRITE,
                               MAP_SHARED, vid->fd, buf.m.offset);
...
void video_Close(struct vid_struct *vid) {
  for (int i = 0; i < vid->n_buffers; ++i) {
    if (-1 == munmap(vid->buffers[i].buf, vid->buffers[i].length))
...
struct img_struct *video_CreateImage(struct vid_struct *vid) {
  struct img_struct *img = (struct img_struct *) malloc(
          sizeof(struct img_struct));
  img->w = vid->w;
  img->h = vid->h;
